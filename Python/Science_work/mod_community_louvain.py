...
def best_partition(graph,
    ...
    dendo = generate_dendrogram(graph,
                                ...
                                random_state)
    if level > len(dendo):
        return partition_at_level(dendo, 0)
    return partition_at_level(dendo, len(dendo) - level)
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        for node in __randomize(graph.nodes(), random_state):
            ...
            for com, dnc in __randomize(neigh_communities.items(), random_state):
                ...
                # Simulate modularity change for each neighboring community
                # temp_status = status.copy()
                # __insert(node, com, dnc, temp_status)
                if len(status.node2com) <= 1000:
                    simulated_mod = __modularity(status, resolution)
                if incr > best_increase:
                    best_increase = incr
                    best_com = com
                # if simulated_mod > new_mod:
                #     new_mod = simulated_mod
                #     best_com = com
                #     best_increase = incr
                # __remove(node, com, dnc, temp_status)
